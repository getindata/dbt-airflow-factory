...
class DbtAirflowTasksBuilderFactory:
    ...
    def create(self) -> DbtAirflowTasksBuilder:
        ...
        return DbtAirflowTasksBuilder(KubernetesPodOperatorBuilder(dbt_execution_env_params, kubernetes_params))

    def _create_dbt_config(self):
        return DbtExecutionEnvironmentParameters(**read_config(self.config_path, self.env, self.dbt_config_file_name))
    ...
    def _prepare_image(config):
        return config['registry'] + '/' + config['repository'] + ':' + str(config['tag'])
